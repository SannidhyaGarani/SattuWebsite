      ...
      */}
      ...
      <div className="container mx-auto px-6 relative z-10 flex flex-col items-center bg-[#faf4e3]">
        <nav 
          ...
          className="flex items-center gap-2.5 md:gap-5 text-[#203B23] font-poppins font-bold text-[10px] md:text-xs uppercase tracking-[0.2em] md:tracking-[0.25em]"
        >
          {breadcrumbItems.map((item, index) => {
            ...
            if (isLast) {
              return (
                <React.Fragment key={index}>
                  {index > 0 && (
                    <ChevronRight size={13} className="text-[#203B23]/40" strokeWidth={3} />
                  )}
                  <motion.div 
            ...
            return (
              <React.Fragment key={index}>
                ...
                >
                  {isFirst && (
                     <Home size={18} className="text-[#203B23] group-hover:-translate-y-0.5 transition-transform" strokeWidth={2} />
                  )}
